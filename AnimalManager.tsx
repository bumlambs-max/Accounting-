        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleAddSpecies} className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-slate-800">{editingSpecies ? 'Edit Species' : 'Add New Animal Species'}</h3>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredSpecies.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-400 italic">{species.length === 0 ? "No animal records found." : "No animals match your filters."}</td></tr>
                ) : (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {sortedHistory.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-400 italic">{logs.length === 0 ? "No activity logs recorded yet." : "No logs match your search."}</td></tr>
                ) : (
