...
import { Liability, LiabilityCategory, Category, Transaction, TransactionType, Account } from '../types.ts';
...
const LiabilityManager: React.FC<LiabilityManagerProps> = ({ 
  ...
  // Fix: Added state for payment account
  ...
  const handleRecordPayment = (e: React.FormEvent) => {
    ...
    // Fix: Added accountId to satisfy Omit<Transaction, 'id'> interface
    onAddTransaction({
      date: paymentDate,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => { setShowForm(true); setEditingLiability(null); setNewLiability({ name: '', category: LiabilityCategory.LOAN, originalAmount: 0, currentBalance: 0, interestRate: 0, dueDate: '', description: '' }); }}
          ...
          <form onSubmit={handleRecordPayment} className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mb-4">Current Balance: ${showPayModal.currentBalance.toLocaleString()}</p>
              </div>
              <div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredLiabilities.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-400 italic">{liabilities.length === 0 ? "No liabilities recorded." : "No liabilities match your search."}</td></tr>
              ) : (
